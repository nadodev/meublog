                ...
                <div class="comment p-4 bg-[#2e2e33] rounded mb-4">
                        ...
                        <div class="flex items-center gap-2 justify-between">
                            ...
                            @if ($comment->user_id === Auth::id())
                                <form action="{{ route('comment.destroy', $comment) }}" method="POST"
                                    ...
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="px-4 py-2 bg-transparent text-white"><i
                    ...
                    @foreach ($comment->replies as $reply)
                        <div class="comment--reply p-4 bg-[#39393c] border-[#464646] rounded mt-4 ml-6 border">
                            <div class=" flex items-center gap-2 justify-between">
                                <p><strong class="text-zinc-500 font-semibold">{{ $reply->user->name }} respondeu:
                                    </strong>
                                </p>
                                <!-- Botão de Exclusão para Respostas -->
                                @if ($reply->user_id === Auth::id())
                                    <form action="{{ route('comment.destroy', $reply) }}" method="POST"
                                        class="mt-2 delete-comment-form">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="px-4 py-2 bg-transparent text-white"><i
                                                class="ri-delete-bin-5-line"></i></button>
